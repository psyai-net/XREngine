...
#include "RuntimeTransformEditor/TransformerPawn.h"
...
/* Gizmos */
#include "RuntimeTransformEditor/Gizmos/BaseGizmo.h"
#include "RuntimeTransformEditor/Gizmos/TranslationGizmo.h"
#include "RuntimeTransformEditor/Gizmos/RotationGizmo.h"
#include "RuntimeTransformEditor/Gizmos/ScaleGizmo.h"

/* Interface */
#include "RuntimeTransformEditor/FocusableObject.h"
...
void ATransformerPawn::SetupPlayerInputComponent(UInputComponent* InInputComponent)
{
	//PublicContactInitializeDefaultPawnInputBindings();

	InInputComponent->BindAxis("DefaultPawn_MoveForward", this, &ATransformerPawn::CustomMoveForward);
	InInputComponent->BindAxis("DefaultPawn_MoveRight", this, &ATransformerPawn::CustomMoveRight);
	InInputComponent->BindAxis("DefaultPawn_MoveUp", this, &ATransformerPawn::CustomMoveUp_World);
	InInputComponent->BindAxis("DefaultPawn_Turn", this, &ATransformerPawn::CustomTurnAt);
	InInputComponent->BindAxis("DefaultPawn_LookUp", this, &ATransformerPawn::CustomLookUpAt);
}
...
void ATransformerPawn::FilterHits(TArray<FHitResult>& outHits)
{
	...
	if (bIgnoreNonReplicatedObjects)
	{
		for (auto Iter = outHits.CreateIterator(); Iter; ++Iter)
		{
			...
			if (IterActor && Iter->Component.IsValid())
			{
				UE_LOG(LogRuntimeTransformer, Warning, TEXT("Removing (Actor: %s   ComponentHit:  %s) from hits because it is not supported for networking.")
					, *IterActor->GetName(), *Iter->Component->GetName());
			}
	...
	/*if (bIgnoreCustomUseActor)
	{
		for (auto IterTemp = outHits.CreateIterator(); IterTemp; ++IterTemp)
		{
			if (Cast<ABaseGizmo>(IterTemp->GetActor()))
				continue;
			AActor* IterActor = IterTemp->GetActor();
			if (IterActor->Tags.Find(TEXT("transform")) == -1)
			{
				IterTemp.RemoveCurrent();
			}
		}
	}*/
}
...
void ATransformerPawn::CustomMoveForward(float Val)
{
	if (Val != 0.f)
	{
		if (Controller)
...
void ATransformerPawn::CustomMoveRight(float Val)
{
	if (Val != 0.f)
	{
		if (Controller)
...
void ATransformerPawn::CustomMoveUp_World(float Val)
{
	if (Val != 0.f)
	{
...
void ATransformerPawn::CustomTurnAt(float Rate)
{
...
void ATransformerPawn::CustomLookUpAt(float Rate)
{
	if (Rate != 0.f && Controller && Controller->IsLocalPlayerController())
	{
		APlayerController* const PC = CastChecked<APlayerController>(Controller);
