...
#include "PublicContactPlugins.h"
...
UCLASS()
class PUBLICCONTACTPLUGINS_API ATransformerPawn : public APawn
{
	GENERATED_BODY()
...
private:
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Replicated Runtime Transformer", meta = (AllowPrivateAccess = "true"))
		bool bIgnoreCustomUseActor = true;
...
public:
	UFUNCTION()
		void CustomMoveForward(float Val);
	UFUNCTION()
		void CustomMoveRight(float Val);
	UFUNCTION()
		void CustomMoveUp_World(float Val);
	UFUNCTION()
		void CustomTurnAt(float Rate);
	UFUNCTION()
		void CustomLookUpAt(float Rate);
};
